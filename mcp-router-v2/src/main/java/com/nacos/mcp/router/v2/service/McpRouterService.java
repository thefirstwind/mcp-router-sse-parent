...
public class McpRouterService {
    ...
    public Mono<McpMessage> routeRequest(String serviceName, McpMessage message, Duration timeout) {
        ...
        return serverRegistry.getAllHealthyServers(serviceName, "mcp-server")
                .next() // 取第一个健康的服务器
                .switchIfEmpty(Mono.error(new IllegalStateException("No healthy instances available for service: " + serviceName)))
                .flatMap(serverInfo -> {
                    // 解析工具调用参数
                ...
                .timeout(timeout)
                .doOnNext(response -> log.info("Successfully routed request to service: {}, response received", serviceName))
    ...
    public Mono<Object> listServerTools(String serviceName) {
        ...
        return serverRegistry.getAllHealthyServers(serviceName, "mcp-server")
                .next()
                .switchIfEmpty(Mono.error(new IllegalStateException("No healthy instances available for service: " + serviceName)))
                .flatMap(mcpClientManager::listTools)
                .map(result -> (Object) result.tools())
                .doOnSuccess(tools -> log.info("Listed {} tools for service: {}", ((java.util.List<?>) tools).size(), serviceName))
    ...
    public Mono<Boolean> hasServerTool(String serviceName, String toolName) {
        ...
        return serverRegistry.getAllHealthyServers(serviceName, "mcp-server")
                .next()
                .switchIfEmpty(Mono.error(new IllegalStateException("No healthy instances available for service: " + serviceName)))
                .flatMap(serverInfo -> mcpClientManager.hasTool(serverInfo, toolName))

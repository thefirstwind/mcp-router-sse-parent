# Dify 集成 MCP Bridge v3 详细指南

## 📋 概述

本指南详细说明如何在 Dify 中集成 MCP Bridge v3 的两个核心接口，实现智能工具发现和调用功能。

### 核心接口说明
- **接口1**: `/mcp/bridge/tools/{serviceName}` - 获取服务的工具列表
- **接口2**: `/mcp/bridge/route/{serviceName}` - 调用具体工具
...
**进入 Dify 工作台 → 工具 → 自定义工具 → 创建工具**

**基础信息**:
```yaml
工具名称: MCP_Bridge_Tool_Discovery
